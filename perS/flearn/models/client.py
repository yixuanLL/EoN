...
class Client(object):
    ...
    def __init__(self, id, epsilon=None, group=None, dataset_name=None, model_name=None,
    ...
    def solve_inner(self, num_epochs=10, batch_size=7, user_sampler=1):
        ...
        bytes_w = self.model.size
        soln, comp = self.model.solve_inner(self.train_data, num_epochs, batch_size, user_sampler)
        bytes_r = self.model.size
        return (self.num_samples, soln), (bytes_w, comp, bytes_r)
    
    def solve_inner_silo(self, pad_mod, pad_sample, up_grads):
        '''Solves local optimization problem
        
        Return:
            1: num_samples: number of samples used in training
            1: solns: local optimization solutions
            2: bytes read: number of bytes received
            2: comp: number of FLOPs executed in training process
            2: bytes_write: number of bytes transmitted  # communication costs
            (bytes_write here count for the full model, without couting the communication reduction by sparsification)
        ...
        bytes_w = self.model.size
        soln, comp = self.model.solve_inner_silo(self.train_data, pad_mod, pad_sample, up_grads)
        bytes_r = self.model.size
        return (self.num_samples, soln), (bytes_w, comp, bytes_r)
    ...
    def test_per_class(self):
        ...
        Return:
            ...
            test_samples: int
        '''
        class_correct, class_sample_num, tot_correct, loss = self.model.test_per_class(self.eval_data)
        return class_correct, class_sample_num, tot_correct, self.test_samples