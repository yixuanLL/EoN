...
class Model(object):
    ...
    def __init__(self, num_classes, optimizer, seed=1):
        ...
        with self.graph.as_default():
            ...
            if 0:
                print('====model2=====')
                self.features, self.labels, self.train_op, self.grads, self.eval_metric_ops, self.loss = self.create_model2(
                optimizer)
            else:
                self.features, self.labels, self.train_op, self.grads, self.eval_metric_ops, self.loss = self.create_model(
                optimizer)
    ...
    def create_model(self, optimizer):
        ...
        pool = tf.layers.max_pooling2d(inputs=conv1, pool_size=[6,6], strides=6)
        pool_flat = tf.reshape(pool, [-1, 4*4*4])
        # pool = tf.layers.max_pooling2d(inputs=conv1, pool_size=[4,4], strides=4)
        # pool_flat = tf.reshape(pool, [-1, 6*6*4])
        dense2 = tf.layers.dense(inputs=pool_flat, units=32, activation=tf.nn.relu)
    ...
    def create_model2(self, optimizer):
        """Model function for Nerual Network."""
        features = tf.placeholder(tf.float32, shape=[None,784], name='features')
        labels = tf.placeholder(tf.int64, shape=[None, ], name='labels')
