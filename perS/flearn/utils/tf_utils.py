...
def fill_grad(flattern, weight):
    weight = np.array(weight)
    num_layers = weight.shape[0]
    filled_weights = [[] for i in range(num_layers)]
    i = 0
    for layer in range(num_layers):
        num_elem = weight[layer].reshape(-1).shape[0]
        filled_weights[layer] = flattern[i: i+num_elem].reshape(weight[layer].shape)
        i += num_elem
